...
abstract public class NTrip {
	...
	private ServiceConnection mConnection = new ServiceConnection() {
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			try {
				...
				NTrip.this.onServiceConnected();//este es el que define el activity
	...
	private void CheckIfServiceIsRunning() {
		...
		if (NTRIPService.isRunning()) {
			...
			LogMessage("Service not yet running");
		}
	}
	...
	public void Connect() {
		...
		if (NTRIPService.isRunning()) {
			if(doBindService())
				LogMessage("Bound Service");
			else
				LogMessage("Failed to bind Service");
		}
	}
	...
	private void LogMessage(String m) {
		 android.util.Log.e("ntrip","LogMessage: "+m);
	}
	...
	boolean doBindService() {
		...
		try {
			...
			LogMessage("Service could not BIND "+e.toString());
			return false;
		}
		mIsBound = true;
		if (mService != null) {
			try {
				...
				Log.e("ntrip",e.toString());
			}
		} else {
			Log.e("ntrip","se hizo el bind pero mService es null?");
		}
		return true;
	}
